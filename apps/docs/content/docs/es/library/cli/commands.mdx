...
Productos: 5
  - prd_c2JV8SjRxSD2y6cRSMzYJPJ2
  - prd_fnvJZwETRrYTmt9y42tLQEMH
  - prd_D1gc8fodQWvUFmJe5YP3IMb1
```

**Casos de uso:**
- Vista rápida de configuración del proyecto
- Verificar tipos de evento, asiento y productos antes de pull
...
Generar definiciones de tipos TypeScript desde tu configuración de Commet. Incluye tipos de evento, tipos de asiento y productos.

<Callout type="info">
**IDs de Productos**: Los productos usan IDs largos (como `prd_D1gc8fodQWvUFmJe5YP3IMb1`) en lugar de códigos cortos. El archivo de tipos generado incluye comentarios que mapean cada ID de producto a su nombre y descripción para fácil referencia.
</Callout>
...
// .commet/types.d.ts
/**
 * Productos disponibles en tu organización:
 * - "prd_c2JV8SjRxSD2y6cRSMzYJPJ2": Implementation Service - Configuración única
 * - "prd_fnvJZwETRrYTmt9y42tLQEMH": API Usage - Llamadas API por uso
 * - "prd_D1gc8fodQWvUFmJe5YP3IMb1": Plan pro
 */
declare module '@commet/node' {
  interface CommetGeneratedTypes {
    ...
    productId: "prd_c2JV8SjRxSD2y6cRSMzYJPJ2" | "prd_fnvJZwETRrYTmt9y42tLQEMH" | "prd_D1gc8fodQWvUFmJe5YP3IMb1";
  }
}
```
...
await commet.seats.add({
  customerId: 'cus_123',
  seatType: 'admin', // ¡Autocompletado funciona!
  count: 5
});

// Suscripciones type-safe
await commet.subscriptions.create({
  customerId: 'cus_123',
  productId: 'prd_c2JV8SjRxSD2y6cRSMzYJPJ2', // ¡Autocompletado con tus IDs de productos!
  status: 'active'
});
```
...
- La organización debe tener tipos de evento, tipos de asiento o productos configurados

<Callout type="info">
**Integración con Git**: Agrega `.commet/types.d.ts` al control de versiones para que los miembros del equipo tengan tipos consistentes.
</Callout>
