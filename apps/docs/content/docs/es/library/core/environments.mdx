...
const commet = new Commet({
  ...
  environment: 'sandbox' // Por defecto - usa api.sandbox.commet.co
})
...
const commet = new Commet({
  apiKey: 'ck_xxx1234567890abcdef',
  environment: 'production' // Usa api.commet.co
})
...
// Hace peticiones API reales a api.sandbox.commet.co
...
// Hace peticiones API reales a api.commet.co
const result = await commet.usage.events.create({
  eventType: 'api_call',
