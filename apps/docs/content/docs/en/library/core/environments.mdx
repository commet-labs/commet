...
const commet = new Commet({
  ...
  environment: 'sandbox' // Default - uses api.sandbox.commet.co
})
...
const commet = new Commet({
  apiKey: 'ck_xxx1234567890abcdef',
  environment: 'production' // Uses api.commet.co
})
...
// Makes real API requests to api.sandbox.commet.co
...
// Makes real API requests to api.commet.co
const result = await commet.usage.events.create({
  eventType: 'api_call',
