...
Each customer has only one active subscription. This returns it with full summary:
...
<Callout type="info">
When `prorate: true`, Commet calculates the credit for unused time on the old plan and the charge for the remaining time on the new plan.
</Callout>
...
// 4. Check current status (includes summary)
