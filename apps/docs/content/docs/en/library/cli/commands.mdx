...
- Verify event types, seat types, and products before pull
...
Generate TypeScript type definitions from your Commet configuration. Includes event types, seat types, and products.

<Callout type="info">
**Product IDs**: Products use long IDs (like `prd_D1gc8fodQWvUFmJe5YP3IMb1`) rather than short codes. The generated types file includes comments mapping each product ID to its name and description for easy reference.
</Callout>
 ...
 * - "prd_c2JV8SjRxSD2y6cRSMzYJPJ2": Implementation Service - One-time setup
 * - "prd_fnvJZwETRrYTmt9y42tLQEMH": API Usage - Pay-per-use API calls
...
await commet.seats.add({
  customerId: 'cus_123',
  seatType: 'admin', // Autocomplete works!
  count: 5
});
...
await commet.subscriptions.create({
  customerId: 'cus_123',
  productId: 'prd_c2JV8SjRxSD2y6cRSMzYJPJ2', // Autocomplete with your product IDs!
  status: 'active'
});
```
...
- Organization must have event types, seat types, or products configured

<Callout type="info">
**Git Integration**: Add `.commet/types.d.ts` to version control so team members have consistent types.
</Callout>
