...
- d2010c1: Align the CLI with v8 by adding Offers, pricing Market Groups, selectable price variants, Offer-aware subscription flows, and the current customer and subscription fields.
...
- 904147f: Fix CLI flags that produced requests the API rejected or ignored:
...
- ec26b3b: fix(listen): accept full URLs without an explicit port. `commet listen https://app.localhost/webhooks` now works, so apps served behind named-host dev proxies (portless and similar) can receive forwarded events. A port is still required for the bare `host:port` shorthand.

## 3.1.1

### Patch Changes

- Updated dependencies [00e5e6b]
  - @commet/node@7.3.0
...
- ada3c5e: Add resource commands to CLI and fix API key prefix.
...
- 00e83a1: Add update notification to CLI. Checks npm registry every 24h and shows a message when a newer version is available. Skipped in agent mode, CI, or when `COMMET_NO_UPDATE_CHECK=1` is set.

## 2.0.0

### Major Changes

- 8353a7c: Agent-first CLI redesign with COMMET_API_KEY support and telemetry.
...
- c2bcef7: Improve CLI for both human and agent use
...
- 8fa823e: `commet create` now supports fully non-interactive usage for CI, scripts, and AI coding agents. Every prompt has an equivalent flag, and missing required inputs fail fast with a helpful message instead of hanging.
...
- 127c714: Fix `commet -V` and `commet --help` incorrectly printing "Error: 1.5.0" after valid output. Commander throws typed exit exceptions for version/help flags; the handler now checks the error `code` instead of the message.
