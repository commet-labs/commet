...
export const pullCommand = new Command('pull')
  ...
  .action(async (options: { output: string }) => {
    ...
    if (!projectConfigExists()) {
      ...
      console.log(chalk.dim('Run `commet link` first to connect to an organization'));
      return;
    }
    ...
    console.log(
      chalk.dim(`  Event types: ${eventTypes.length > 0 ? eventTypes.map((e) => e.code).join(', ') : 'none'}`),
    );
    console.log(
      chalk.dim(`  Seat types: ${seatTypes.length > 0 ? seatTypes.map((s) => s.code).join(', ') : 'none'}`),
    );
