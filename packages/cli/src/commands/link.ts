...
`,
  ...
  .action(async (options: LinkOptions) => {
    ...
    let autoApiKey: string | null = null;
    const keyResult = await apiRequest<{ success: boolean; apiKey: string }>(
    ...
    if (agentMode) {
      console.log(
        JSON.stringify({
          ...
          ...(autoApiKey
            ? { apiKey: autoApiKey, apiKeyAutoGenerated: true }
            : {}),
        }),
      );
    } else {
      ...
      if (autoApiKey) {
        console.log(
          chalk.green("✓ API key auto-generated for resource commands"),
        );
      } else {
        console.log(
          chalk.dim(
